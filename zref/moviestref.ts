...
// import { mockMovies, actionMovies, romanceMovies, dramaMovies, moviesAsResp, getTopRatedMovies, lowestRatedMovie } from './movies.mockData';
...
//     let app: Application;
//     let mockMoviesAsResp: IMovieResponse[], actionMoviesAsResp: IMovieResponse[], dramaMoviesAsResp: IMovieResponse[], romanceMoviesAsResp: IMovieResponse[];
...
//         poolQuerySpy = jest.spyOn<Pool, any>(pool, 'query');
//     });
...
//             expect(poolQuerySpy).toHaveBeenCalledWith('SELECT * FROM movies GROUP BY type, movie_id;');
...
//             expect(response.body.movies.action).toEqual(expect.arrayContaining(actionMoviesAsResp));
//             expect(response.body.movies.drama).toEqual(expect.arrayContaining(dramaMoviesAsResp));
//             expect(response.body.movies.romance).toEqual(expect.arrayContaining(romanceMoviesAsResp));
//         });

//         it('should return movies of a specific category', async () => {
//             const category = 'action';
//             const response = await request(app).get('/movies').query({ category });
//             expect(poolQuerySpy).toHaveBeenCalledWith('SELECT * FROM movies WHERE type = $1 ORDER BY release_date DESC;', [category]);
//             expect(response.status).toBe(statusCodes.success);
//             expect(response.body).toHaveProperty('movies');
//             expect(response.body.movies).toEqual(expect.arrayContaining(actionMoviesAsResp));
//         });
...
//             expect(response.body).toEqual({ error: 'Exception occurred while fetching movies' });
//         });
//     });
...
//             expect(poolQuerySpy).toHaveBeenCalledWith('SELECT * FROM movies ORDER BY rating DESC LIMIT 10;');
//             expect(response.status).toBe(statusCodes.success);
//             expect(response.body).toHaveProperty('movies');
//             expect(response.body.movies).toEqual(expect.arrayContaining(topRatedMovies));
...
//             const response = await request(app).get('/movies/top');
//             expect(response.status).toBe(statusCodes.queryError);
//             expect(response.body).toEqual({ error: 'Exception occurred while fetching top rated movies' });
//         });
//     });
...
//             expect(poolQuerySpy).toHaveBeenCalledWith('SELECT * FROM seen_movies S JOIN movies M ON S.movie_id = M.movie_id WHERE email = $1;', ['johndoe@email.com']);
//             expect(response.status).toBe(statusCodes.success);
//             expect(response.body).toHaveProperty('movies');
...
//             const response = await request(app).get('/movies/me');
//             expect(response.status).toBe(statusCodes.queryError);
//             expect(response.body).toEqual({ error: 'Exception occurred while fetching seen movies' });
